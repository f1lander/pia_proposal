(function () {
	...
	angular.module('memory.controllers', [])
		...
		.controller('AppCtrl', ['$scope', '$ionicModal', '$ionicPopup', '$state', 'Settings', function ($scope, $ionicModal, $ionicPopup, $state, Settings) {
			...
			$scope.showConfirm = function () {
				...
				var confirmPopup = $ionicPopup.confirm({
					title: '¿Está segura que ya no quiere jugar amor?',
					cancelText: 'Yes',
		...
		.controller('ProposalCtrl', ['$scope', '$state','Settings', '$ionicPopup', function ($scope, $state, Settings, $ionicPopup) {

			$scope.SIIIIII = function(){
				var yesPopup = $ionicPopup.confirm({
					title: '<h4>Gracias por seguir haciendome feliz! Te Amo!!</h4>',
					cancelText: 'Menu',
				...
				yesPopup.then(function (res) {
					if (res) {
						console.log('Otra vez?');
						$state.go("game-easy", {}, { reload: true });
					} else {
						console.log('Menu');
						$state.go('home');
					}
			...
			$scope.BUUU = function(){
				var buuPopup = $ionicPopup.confirm({
					title: '<h4>Puque noo?????</h4>',
					cancelText: 'Otra vez',
					cancelType: 'button-dark',
					okText: 'Otra vez',
					okType: 'button-balanced',
		...
		.controller('GameCtrl', ['$scope', '$state', '$ionicPopup', 'icons', 'Settings', function ($scope, $state, $ionicPopup, icons, Settings) {
			...
			$scope.showProposal = function () {
				if (!$scope.proposalModal) {
					// Load the modal from the given template URL
					$ionicModal.fromTemplateUrl('templates/proposal.html', function (modal) {
			...
			$scope.$on('memoryGameUnmatchedPairEvent', function () {
				$scope.message = 'Otra vez!';
				console.log($scope.message);
			});
			$scope.$on('memoryGameMatchedPairEvent', function () {
				$scope.message = 'Sos otro rollo miamor <3!';
				console.log($scope.message);
			});
			...
			$scope.showFinale = function () {
				var finalePopup = $ionicPopup.confirm({
					title: '<h2>Felicidades!</h2><h4>Ganaste mi Poita!</h4>',
					cancelText: 'Menu',
					cancelType: 'button-dark',
					okText: 'Siguiente',
					okType: 'button-balanced',
				...
				finalePopup.then(function (res) {
					if (res) {
						console.log('Otra vez');
						if ($state.current.name == "game-easy") {
							$state.go("game-difficult", {}, { reload: true });
						} else if ($state.current.name == "game-difficult") {
							$state.go("game-hard", {}, { reload: true });
						} else {
							$state.go("proposal", {}, { reload: true });
							//$scope.showProposal();
						}
			...
			$scope.showIconError = function () {
				var errorPopup = $ionicPopup.confirm({
					title: '<h2>ERROR:</h2><h4>Un error amor :( lo siento .</h4>',
					cancelText: 'Menu',
					cancelType: 'button-dark',
					okText: 'Otra vez?',
					okType: 'button-balanced',
